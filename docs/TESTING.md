...
| Frontend | `frontend/src/**/*.test.{ts,tsx}` (24 files) | 68 | — (fetch mocked at the client boundary) | — | Vitest + Testing Library: shared components (`DataTable`, `GanttChart` incl. compare ghosts and clustering, `ExplanationPanel`, `ScoreBar`), auth/client/formatters, and page behaviour for the priority queue (Why? drawer), order detail (explanation lines, reason-required dialogs), priority configuration (weight normalisation, preview), alerts, audit diff, machine detail, system administration (sync), executive dashboard, control tower (plan actions by role, replan comparison), machine schedule list format, capacity table, what-if builder (exact request body) |
