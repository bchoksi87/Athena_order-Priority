...
the operation's process type. Every candidate is then checked against every
hard constraint so a rejected machine carries *all* of its reasons.
"""
...
def candidate_machines(
    ...
    if order is not None and order.required_machine_id is not None:
