...
import type { MachineOption, OverridePriorityKind } from "@/api/types";
...
import { localInputToIso, specFor } from "./orderActionSpecs";
...
export function OrderActionDialog({ orderId, action, onClose, eligible, scheduledMachineId, defaultMachineId, onSuccess }: OrderActionDialogProps) {
  ...
  const requestKey = `${action ?? ""}:${defaultMachineId ?? ""}`;
  const [lastRequestKey, setLastRequestKey] = useState(requestKey);
  // A new move/lock request (from the machine options table) pre-selects its machine; adopt it during render.
